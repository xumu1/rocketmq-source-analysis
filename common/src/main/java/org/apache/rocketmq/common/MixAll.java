...
public class MixAll {
    ...
    // 持久化都是这么做的，先搞一个 tmp，然后 rename（NSQ 也是这么做的）
    public static void string2File(final String str, final String fileName) throws IOException {
        ...
        if (prevContent != null) {
            // 如果是覆盖写，将之前的内容写入 fileName.bak bak 就是 backup 的意思
    ...
    // 读取 fileName 指代的文件，转成 String
    public static String file2String(final String fileName) throws IOException {
        File file = new File(fileName);
        return file2String(file);
    }

    // FileInputStream 读取数据到 byte 数组，再转为 String 对象
    ...
    // 利用 Java 反射将 Object 转换为 Properties
        ...
        for (Method method : methods) {
            ...
            if (mn.startsWith("set")) {
                try {
                    ...
                    String key = first.toLowerCase() + tmp;  // property 的 name
                    String property = p.getProperty(key);
                    if (property != null) {
                        Class<?>[] pt = method.getParameterTypes();
                        if (pt != null && pt.length > 0) {
                            String cn = pt[0].getSimpleName();  // 根据 method 参数的类型，对 property 进行转型
                            Object arg = null;
